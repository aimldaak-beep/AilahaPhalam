    ...
    <div style={{
              ...
              return (
                <div key={trade.id} className="p-4 rounded-lg flex flex-col justify-between gap-3.5 shadow-md" style={{background:'rgba(4,2,0,0.95)',border:'1px solid rgba(201,168,76,0.08)'}}>
                  <div className="flex items-center justify-between text-xs">
                    <div>
                      <span className="font-black font-mono text-lg tracking-wide" style={{color:'#F0E6C8'}}>
                        {trade.symbol}
                      </span>
                    ...
                    <div className="text-right">
                      <span className="block font-mono font-bold text-[15px]" style={{color:'rgba(240,230,200,0.7)'}}>Entry: {currencySymbol}{formatNumber(entryPrice)}</span>
                    </div>
                  </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, padding: '20px 28px 0', marginBottom: 16 }}>
        ...
        <div id="weekly-net-card" style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 16, padding: '20px 24px', position: 'relative' }}>
          ...
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.1, color: adjustedWeeklyNet >= 0 ? '#677A67' : '#C9960C' }}>
        ...
        <div id="weekly-gross-card" style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 16, padding: '20px 24px', position: 'relative' }}>
          ...
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.1, color: weeklyGrossSum >= 0 ? '#677A67' : '#C9960C' }}>
        ...
        <div id="weekly-brokerage-card" style={{ background: 'rgba(255,255,255,0.03)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 16, padding: '20px 24px', position: 'relative' }}>
          ...
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.5px', lineHeight: 1.1, color: '#C9960C' }}>
      ...
      <div id="weekly-standing-card" style={{ margin: '0 28px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12, padding: '16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          <div className="rounded-lg p-3.5" style={{background:'rgba(5,3,1,0.95)',border:'1px solid rgba(201,168,76,0.15)'}}>
            ...
            <span id="standing-realized" className="font-mono font-black text-lg" style={{color: weeklyStanding.realized >= 0 ? '#F0E6C8' : '#C9960C'}}>
          ...
          <div className="rounded-lg p-3.5" style={{background:'rgba(5,3,1,0.95)',border:'1px solid rgba(201,168,76,0.15)'}}>
            ...
            <span id="standing-mtm" className="font-mono font-black text-lg" style={{color: weeklyStanding.mtm >= 0 ? '#F0E6C8' : '#C9960C'}}>
          ...
          <div className="rounded-lg p-3.5" style={{background:'rgba(201,168,76,0.05)',border:'1px solid rgba(201,168,76,0.3)'}}>
            ...
            <span id="standing-net" className="font-mono font-black text-2xl" style={{color: weeklyStanding.net >= 0 ? '#677A67' : '#C9960C'}}>
      ...
      <div id="week-offset-card" style={{ margin: '0 28px 16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12, padding: '16px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          <div className="rounded-lg p-3.5" style={{background:'rgba(5,3,1,0.95)',border:'1px solid rgba(201,168,76,0.15)'}}>
            ...
            <span className="font-mono font-black text-lg" style={{color: weeklyNetSum >= 0 ? '#F0E6C8' : '#C9960C'}}>
          ...
          <div className="rounded-lg p-3.5" style={{background:'rgba(5,3,1,0.95)',border:'1px solid rgba(201,168,76,0.15)'}}>
            ...
            <span className="font-mono font-black text-lg" style={{color: savedOffsetAmount === 0 ? 'rgba(240,230,200,0.5)' : savedOffsetAmount > 0 ? '#677A67' : '#C9960C'}}>
          ...
          <div className="rounded-lg p-3.5" style={{background:'rgba(201,168,76,0.05)',border:'1px solid rgba(201,168,76,0.3)'}}>
            ...
            <span className="font-mono font-black text-xl" style={{color: adjustedWeeklyNet >= 0 ? '#677A67' : '#C9960C'}}>
              {adjustedWeeklyNet >= 0 ? '+' : ''}₹{formatAmount(adjustedWeeklyNet)}
            </span>
          </div>
        </div>
        ...
        {openTrades.length === 0 ? (
          ...
          <div style={{ background: 'rgba(0,0,0,0.15)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12, overflow: 'hidden', marginTop: 12 }}>
            <div style={{ minWidth: 1340 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '195px 120px 110px 1fr 220px 155px 175px 160px', padding: '8px 20px', background: 'rgba(201,168,76,0.05)', borderBottom: '1px solid rgba(201,168,76,0.12)', gap: 8 }}>
              ...
              {openTrades.map((trade) => {
                ...
                const existingFx = trade.fridayUsdToInrRates?.[selectedWeekKey];
                return (
                  <div key={trade.id} id={`active-trade-${trade.id}`} style={{ display: 'grid', gridTemplateColumns: '195px 120px 110px 1fr 220px 155px 175px 160px', padding: '16px 20px', borderBottom: '1px solid rgba(201,168,76,0.06)', alignItems: 'center', gap: 8 }}>
                    <div className="font-mono" style={{ fontSize: 18, fontWeight: 700, color: '#F0E6C8', letterSpacing: '0.3px' }}>{trade.symbol}</div>
                    <div>
                      ...
                      <span className="block font-mono mt-0.5" style={{ fontSize: 14, color: 'rgba(240,230,200,0.5)' }}>{trade.instrument}</span>
                    ...
                    <div className="font-mono whitespace-nowrap" style={{ fontSize: 17, color: 'rgba(240,230,200,0.7)' }}>{trade.numberOfLots} × {trade.lotSize}</div>
                    {/* Merged entry price + entry date cell */}
                    <div>
                      <div className="font-mono whitespace-nowrap" style={{ fontSize: 18, fontWeight: 700, color: '#F0E6C8' }}>{sym}{formatPrice(entryPrice)}</div>
                      <div className="font-mono whitespace-nowrap" style={{ fontSize: 13, color: 'rgba(240,230,200,0.5)' }}>{trade.dateInitiated}</div>
                    </div>
                      ...
                      {wkCalc.isActive ? (
                        <>
                          <input
                            ...
                            className="font-mono"
                            style={{ width: 98, background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 8, padding: '7px 8px', color: '#F0E6C8', fontSize: 14, outline: 'none' }}
                          />
                          {trade.currency === 'USD' && (
                            <input
                              type="text"
                              ...
                              className="font-mono"
                              style={{ width: 60, background: 'rgba(201,168,76,0.04)', border: '1px solid rgba(201,168,76,0.15)', borderRadius: 8, padding: '7px 6px', color: '#F0E6C8', fontSize: 14, outline: 'none' }}
                            />
                          )}
                    ...
                    <div className="font-mono" style={{ textAlign: 'right' }}>
                      {wkCalc.isActive ? (
                        <span style={{ fontSize: 18, fontWeight: 700, color: wkCalc.netProfit >= 0 ? '#677A67' : '#C9960C' }} title={`Week ${activeWeekInfo.weekNum} mark-to-market net`}>
                    ...
                    <div className="font-mono" style={{ textAlign: 'right' }}>
                      {curPnl === null ? (
                        ...
                        <span style={{ fontSize: 18, fontWeight: 700, color: curPnl >= 0 ? '#677A67' : '#C9960C' }} title={live ? 'Live — from your entered current price' : 'Mark-to-date — last recorded weekly closes'}>
        ...
        {closedInWeek.length === 0 ? (
          ...
          <div style={{ background: 'rgba(0,0,0,0.15)', border: '1px solid rgba(201,168,76,0.1)', borderRadius: 12, overflow: 'hidden', marginTop: 12 }}>
            <div style={{ minWidth: 1330 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '205px 130px 110px 150px 150px 110px 165px 180px 80px', padding: '8px 20px', background: 'rgba(201,168,76,0.05)', borderBottom: '1px solid rgba(201,168,76,0.12)', gap: 8 }}>
              ...
              {closedInWeek.map((item) => {
                ...
                return (
                  <div key={trade.id} id={`closed-trade-${trade.id}`} style={{ display: 'grid', gridTemplateColumns: '205px 130px 110px 150px 150px 110px 165px 180px 80px', padding: '16px 20px', borderBottom: '1px solid rgba(201,168,76,0.06)', alignItems: 'center', gap: 8 }}>
                    <div className="font-mono" style={{ fontSize: 18, fontWeight: 700, color: '#F0E6C8', letterSpacing: '0.3px' }}>{trade.symbol}</div>
                    <div>
                      <span className="font-mono" style={{ fontSize: 9, fontWeight: 800, padding: '2px 8px', borderRadius: 4, letterSpacing: '1px', display: 'inline-block', ...(trade.direction === 'Long' ? { background: 'rgba(103,122,103,0.1)', color: '#677A67', border: '1px solid rgba(103,122,103,0.25)' } : { background: 'rgba(201,150,12,0.1)', color: '#C9960C', border: '1px solid rgba(201,150,12,0.25)' }) }}>
                        {trade.direction.toUpperCase()}
                    ...
                    <div className="font-mono whitespace-nowrap" style={{ fontSize: 17, color: 'rgba(240,230,200,0.7)' }}>{trade.numberOfLots} × {trade.lotSize}</div>
                    <div className="font-mono whitespace-nowrap" style={{ fontSize: 18, fontWeight: 700, color: '#F0E6C8' }}>{sym}{formatPrice(entryPrice)}</div>
                    <div className="font-mono whitespace-nowrap" style={{ fontSize: 18, fontWeight: 700, color: '#F0E6C8' }}>{sym}{formatPrice(exitPrice)}</div>
                    <div className="font-mono whitespace-nowrap" style={{ fontSize: 13, color: 'rgba(240,230,200,0.45)', letterSpacing: '0.3px' }}>{exitDate || '—'}</div>
                    <div className="font-mono" style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: 18, fontWeight: 700, color: item.netProfit >= 0 ? '#677A67' : '#C9960C' }} title={`Week ${activeWeekInfo.weekNum} piece of this trade`}>
                        {item.netProfit >= 0 ? '+' : ''}₹{formatAmount(item.netProfit)}
                      </span>
                    </div>
                    <div className="font-mono" style={{ textAlign: 'right' }}>
                      <span style={{ fontSize: 18, fontWeight: 700, color: totNet >= 0 ? '#677A67' : '#C9960C' }} title="Final realized net across the trade's life">
