...
cur = json.loads(urllib.request.urlopen(urllib.request.Request(f'{SB}/storage/v1/object/authenticated/config/allowlist.json', headers=H), timeout=30).read())['emails']  # authenticated = uncached (the public URL is CDN-cached and can re-add a stale email)
