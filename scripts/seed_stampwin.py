#!/usr/bin/env python3
"""Seed for the stamp-law / stamp-window smoke (2026-09-14). Same throwaway-user pattern as seed_journalfix.py:
   DOW-PT  USD Long ×5 opened Tue 25 Aug (W35) @40000 rate 90, stamps W35 40100 / W36 40300, CLOSED Wed 9 Sep @40250 → realized +89,280
   NIF-PT  INR Futures ×75 opened Mon 31 Aug (W36) @24400, stamps W36 24500 / W37 24450, OPEN → +6,951 / −3,750
   NAS-PT  USD Nasdaq ×20 opened Mon 7 Sep (W37) @20000 rate 89, stamp W37 20050, OPEN → +70,844
   TATA-PT INR NSE Futures ×250 opened Wed 9 Sep (W37) @8000, NO stamp (owes W37)
   GIFT-PT INR Futures ×75 opened Mon 7 Sep (W37) @24400, NO stamp (owes W37)
   SUN-PT  INR Futures ×75 opened SUNDAY 13 Sep (calendar W37) @24500, NO stamp — owes NOTHING for W37, first owed week W38
   Writes sessPT.json + pertrade.json into env PT_DIR (cleanup_pertrade.py removes everything)."""
...
email = f'smoke-sw-{random.randint(1,10**9)}@example.com'
...
sun = T(id='t_sun_pt', symbol='SUN-PT', instrument='Futures', direction='Long', dateInitiated='2026-09-13', buyPrice=24500, sellPrice=None, buyDate='2026-09-13', sellDate=None, lotSize=75, numberOfLots=1, status='CarryForwardLong', currency='INR', usdToInrRate=1, realizationRate=1.0)
post(f'{SB}/rest/v1/trades', [{'user_id': uid, 'data': d} for d in (dow, nif, nas, tata, gift, sun)], pref='return=minimal')
...
cur = json.loads(urllib.request.urlopen(urllib.request.Request(f'{SB}/storage/v1/object/authenticated/config/allowlist.json', headers=H), timeout=30).read())['emails']
urllib.request.urlopen(urllib.request.Request(f"{SB}/storage/v1/object/config/allowlist.json", data=json.dumps({'emails': sorted(set(cur + [email]))}).encode(), method='PUT',
    headers={**H, 'Content-Type': 'application/json', 'x-upsert': 'true'}), timeout=30)
